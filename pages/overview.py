...
def render():
    ...
    # ── Individual Model Score Cards ──────────────────────────────────────────
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("### 🤖 Individual Model Scores — Dual-Path Ensemble")
    mc1, mc2, mc3, mc4, mc5 = st.columns(5)
    model_scores = [
        ("Random Forest", "Session Detection", "100.00%", "#00f5a0", "Path A — Detects high-risk sessions using tree ensemble voting"),
        ("Logistic Regression", "Session Detection", "99.67%", "#00c8ff", "Path A — Probability-based session risk scoring"),
        ("Random Forest", "Attacker Profiling", "100.00%", "#a855f7", "Path B — Classifies PERSISTENT vs SCANNER attacker types"),
        ("SVM LinearSVC", "Attacker Profiling", "100.00%", "#ffa500", "Path B — Behavioural support vector classification"),
        ("Ensemble (All 4)", "Final Confidence", "99.43%", "#ff4560", "Weighted: RF×0.40 + LR×0.35 + SVM×0.25"),
    ]
    for col, (name, path, acc, color, desc) in zip([mc1,mc2,mc3,mc4,mc5], model_scores):
        with col:
            st.markdown(f"""
            <div style="background:#0a0f1e;border:1px solid #1a2540;border-top:3px solid {color};
                 border-radius:8px;padding:14px;text-align:center;height:170px;">
              <div style="font-size:0.6rem;color:#4a5a7a;text-transform:uppercase;letter-spacing:1px;">{path}</div>
              <div style="font-family:Rajdhani,sans-serif;font-size:0.9rem;font-weight:700;color:{color};margin:4px 0;">{name}</div>
              <div style="font-family:Rajdhani,sans-serif;font-size:2.2rem;font-weight:700;color:{color};line-height:1;">{acc}</div>
              <div style="background:#1a2540;border-radius:3px;height:5px;margin:8px 0;">
                <div style="background:{color};width:{acc.replace('%','').replace('.','')[:3]}%;height:100%;border-radius:3px;max-width:100%;"></div>
              </div>
              <div style="font-size:0.6rem;color:#4a5a7a;line-height:1.4;">{desc}</div>
